...
export async function POST(request: NextRequest) {
  try {
    ...
    // 加载角色配置
    const allCharacters = await ConfigLoader.loadAllCharacters();
    ...
    // 随机选择角色
    const selectedConfigs = ConfigLoader.selectRandomCharacters(allCharacters, 3, 5);
    const characters = selectedConfigs.map(config => ConfigLoader.configToCharacterCard(config));
    
    // 加载角色事件
    const allEvents = [];
    for (const config of selectedConfigs) {
      const events = await ConfigLoader.loadCharacterEvents(config.id);
      const eventCards = events.map(event => ConfigLoader.configToEventCard(event, config.id));
