...
export async function GET(req: NextRequest) {
  ...
  type CommonCard = {
    ...
    _events?: { id: string; title: string }[];
  };
  const card = allCommonCards.find(c => (c as CommonCard).id === id) as CommonCard | undefined;
  if (!card) {
  ...
  if (Array.isArray(card.events)) {
    events = card.events.filter((e: { id: string; title: string }) => eventIds.includes(e.id));
  } else if (Array.isArray(card._events)) {
    events = card._events.filter((e: { id: string; title: string }) => eventIds.includes(e.id));
  }
